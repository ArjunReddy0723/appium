...
class Doctor {
  ...
  async reportManualFixes () {
    ...
    if (manualFixes.length >0) {
      ...
      log.info('###');
      log.info('');
      log.info('Bye! execute Appium again when all manual fixes have been applied!');
      log.info('');
      return true;
    } else {
      return false;
  ...
  async runAutoFixes () {
    ...
    if (_.find(autoFixes, (f) => { return !f.fixed; })) {
      // a few issues remain.
      log.info('Bye! A few issues remain, fix manually and rerun Appium!');
    } else {
      // nothing left to fix.
  ...
  async run () {
    log.info(`Appium pre-requisite checks ...!`);
    await this.diagnose();
